    ...
    public function generateUrl(): string
    {
        ...
        if (null !== $crudId = $this->get(EA::CRUD_ID)) {
            if (null === $crudControllerFqcn = $this->crudControllerRegistry->findCrudFqcnByCrudId($crudId)) {
                throw new \InvalidArgumentException(sprintf('The given "%s" value is not a valid CRUD ID. Instead of dealing with CRUD controller IDs when generating loginadmin URLs, use the "setController()" method to set the CRUD controller FQCN.', $crudId));
            }
        ...
        if (null === $this->dashboardRoute) {
            if ($this->dashboardControllerRegistry->getNumberOfDashboards() > 1) {
                throw new \RuntimeException('When generating loginadmin URLs from outside EasyAdmin or without a related HTTP request (e.g. in tests, console commands, etc.), if your application has more than one Dashboard, you must associate the URL to a specific Dashboard using the "setDashboard()" method.');
            }
