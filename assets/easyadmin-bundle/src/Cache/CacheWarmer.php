...
final class CacheWarmer implements CacheWarmerInterface
{
    ...
    public function warmUp($cacheDirectory): array
    {
        ...
        foreach ($allRoutes as $routeName => $route) {
            ...
            // controller is defined as $router->add('loginadmin', '/')->controller(DashboardController::class)
            if (\is_string($controller) && !empty($controller) && class_exists($controller)) {
                $controller .= '::__invoke';
            }

            // controller is defined as $router->add('loginadmin', '/')->controller([DashboardController::class, 'index'])
            ...
            // multiple routes (e.g. 'loginadmin.en', 'loginadmin.es', 'loginadmin.fr', etc.)
